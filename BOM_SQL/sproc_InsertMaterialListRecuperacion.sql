...
ALTER PROCEDURE [dbo].[sproc_Recuperation]
 @Code VARCHAR(100),
...
BEGIN
 ...
 BEGIN TRANSACTION Recuperation
 BEGIN TRY
   ...
   BEGIN
     ...
     BEGIN
      UPDATE [Warehouse] SET [StockTotal] = @Total WHERE [Material_FK] = @MaterialId AND Location = @Location AND kLT = @KTL
     END
     ELSE
     BEGIN
      INSERT INTO [Warehouse] ([Material_FK], [StockTotal], [Location], [KLT]) VALUES (@MaterialId, @Total, @Location,@KTL);
 ...
 COMMIT TRANSACTION Recuperation;
 END TRY
 BEGIN CATCH
 SELECT
'ERROR'
   ...
   ROLLBACK TRANSACTION Recuperation;
  END CATCH;
